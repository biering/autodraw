...
export const NODE_CLIP_MARKER = "agentsdraw-node-clip-v1";
export const NODES_CLIP_MARKER = "agentsdraw-nodes-clip-v1";
...
/**
 * Parse clipboard JSON: {@link NODES_CLIP_MARKER} (one or more nodes) or legacy
 * {@link NODE_CLIP_MARKER} (single node).
 */
export function parseAgentsdrawNodesClipboard(text: string): Omit<NodeRecord, "id">[] | null {
  const t = text.trim();
  ...
  if (parsed.$type === NODES_CLIP_MARKER && Array.isArray(parsed.nodes)) {
  ...
  if (parsed.$type === NODE_CLIP_MARKER) {
 ...
 */
export function parseAgentsdrawNodeClipboard(text: string): Omit<NodeRecord, "id"> | null {
  const arr = parseAgentsdrawNodesClipboard(text);
