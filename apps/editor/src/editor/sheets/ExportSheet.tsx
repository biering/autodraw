...
export function ExportSheet() {
  ...
  const onExport = async () => {
    if (!isTauri()) {
      console.warn(
        "[autodraw] PDF/PNG export uses the Rust backend. From the repo root run `pnpm dev:tauri` (install Rust from https://rustup.rs/ if `cargo` is missing).",
      );
      return;
    }
    setBusy(true);
      ...
      <DialogContent className="max-w-md sm:max-w-md" aria-describedby="export-desc">
        ...
        <div className="flex items-center gap-2 pt-1">
          <Checkbox
            id="export-grid"
            checked={showGrid}
            onCheckedChange={(v: CheckedState) => setShowGrid(v === true)}
          />
