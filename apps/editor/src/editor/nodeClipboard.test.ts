import { defaultStyleId, emptyDiagram, type NodeRecord } from "@autodraw/core";
import { describe, expect, it } from "vitest";
import {
  NODE_CLIP_MARKER,
  NODES_CLIP_MARKER,
  parseAutodrawNodeClipboard,
...
describe("parseAutodrawNodeClipboard", () => {
  it("returns null for non-JSON or wrong marker", () => {
    expect(parseAutodrawNodeClipboard("")).toBeNull();
    expect(parseAutodrawNodeClipboard("not json")).toBeNull();
    expect(parseAutodrawNodeClipboard("{}")).toBeNull();
    expect(parseAutodrawNodeClipboard(JSON.stringify({ $type: "other", text: "x" }))).toBeNull();
  });
  ...
  it("parses a valid payload (shape optional)", () => {
    ...
    const parsed = parseAutodrawNodeClipboard(json);
  ...
  it("round-trips through serialize and parse", () => {
    ...
    const parsed = parseAutodrawNodeClipboard(serializeNodeForClip(n));
  ...
  it("returns null for multi-node clipboard (use parseAutodrawNodesClipboard)", () => {
    const a = sampleNode({ id: "a", text: "A", x: 0, y: 0 });
    const b = sampleNode({ id: "b", text: "B", x: 200, y: 0 });
    const json = serializeNodesForClip([a, b]);
    expect(parseAutodrawNodeClipboard(json)).toBeNull();
    const multi = parseAutodrawNodesClipboard(json);
...
describe("parseAutodrawNodesClipboard", () => {
  it("rejects invalid multi payload", () => {
    expect(
      parseAutodrawNodesClipboard(JSON.stringify({ $type: NODES_CLIP_MARKER, nodes: "bad" })),
    ).toBeNull();
    expect(
      parseAutodrawNodesClipboard(JSON.stringify({ $type: NODES_CLIP_MARKER, nodes: [] })),
    ).toBeNull();
  });
  ...
  it("accepts legacy Agentsdraw multi-node marker", () => {
    ...
    const parsed = parseAutodrawNodesClipboard(json);
    expect(parsed).toHaveLength(1);
    expect(parsed![0]!.text).toBe("L");
  });
});
